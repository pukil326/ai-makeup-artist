...
| 页面 | 说明 |
|---|---|
| `/`（[index.html](index.html)） | **实时摄像头**版:开摄像头 → AI 自动上妆 → 风格切换 / 浓度 / 前后对比 / 保存图 |
| `/photo.html` | **照片试妆**版:上传一张人脸照 → AI 诊断 + 上妆 → 导出"前后对比 + 诊断"图(做 demo 素材最稳) |
...
### 1) 配置 API Key
在项目根目录 `.env` 填入(已 gitignore,不会提交):
```
...
- **天花板**:渲染是参数化(关键点+shader),自然但非照片级真实。要"美图级真实"需转**学习式妆容迁移(GAN/扩散)**——另一个量级,留作后续。
- **后续方向**:学习式渲染;用化妆师标注数据微调垂直审美模型(「微调极客」方向);接真实品牌色卡,AI 推荐"可买的同色号"(变现)。

---

*一人 1–2 周做出可演示 MVP。一切妆效由"化妆品"逻辑达成(粉底/遮瑕/修容…),不走美颜滤镜磨皮。*
