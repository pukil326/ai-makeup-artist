...
**关键纪律：绝不每帧调大模型**（太慢太贵）。方案算一次，存住，重用。
...
- **演示脚本**：素颜 → 点一下 → AI 实时上妆 + 弹出诊断理由。3 秒内见效果，评委一眼记住。
- **讲清差异化**：别人是"试妆工具"，你是"AI 化妆师"——把审美决策权交给 AI，这是认知升级。
- **小红书传播钩子**：前后对比图 + AI 一句毒舌/专业点评，是平台最易爆的内容形态。
- **对齐赛道关键词**：一人成军（一人可做）、实效落地（能跑的 demo）、商业化潜力（可接美妆电商/品牌色卡）、能引发复制热潮（人人想试自己的"AI 定妆"）。

---

## 九、后续可扩展方向（先别做，留作答辩亮点）

- 接入真实品牌色卡，AI 推荐"可买的同色号产品"——直接打通变现。
- 多套风格切换（通勤 / 约会 / 氛围感），每套都带 AI 理由。
- 短视频实时美妆（直播 / Vlog 场景）。
- 用化妆师标注数据微调一个垂直模型（对应「微调极客」赛道）。

---

*这份方案以一个人 1–2 周做出可演示 MVP 为目标。建议严格按阶段推进，阶段 1 的渲染观感一旦验证通过，后面都是堆功能。*
